...
If the path info matches this regular expression, instead of using
C<default_expires>, the value C<index_expires> will be used for the expiry
argument to C<set()> and C<handle()>.

    $pinfo =~ m/(?:03modlist\.data|02packages\.details\.txt|01mailrc\.txt)/;
